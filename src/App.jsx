  ...
  return (
    <>
      ...
      <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-[35%]" >
        ...
        <div className='h-[2px] bg-black w-[90%] my-2 mx-auto opacity-20'></div>
          ...
          {todos.map(item => {
            return (showFinished || !item.isCompleted) && <div key={item.id} className={"todo flex my-3 justify-between"}>
              ...
              <div className="buttons flex h-full">
                ...
                <button onClick={(e) => { handleClear(e, item.id) }} className='bg-violet-800 hover:bg-violet-950 p-3 py-1 text-sm font-bold text-white rounded-md mx-1'><MdDelete /></button>
              </div>
            </div>
          })}
        </div>
